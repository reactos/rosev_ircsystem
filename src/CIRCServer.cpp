...
CIRCServer::Init()
{
    ...
    if(m_Configuration.DoUseSSL())
    {
        ...
        m_SSLContext->use_certificate_chain_file(m_Configuration.GetSSLCertificateFile());
        ...
        else
        {
            ...
            else
            {
                ...
                if(Clients.find(Sender) == Clients.end())
                {
                    ...
                    /* Publicly show the mode change */
                    if(IsVoicedClient)
                    {
                        std::string Response(boost::str(boost::format(":%s MODE #%s +v %s") % m_Configuration.GetName() % it->second->GetName() % Sender->GetNickname()));
